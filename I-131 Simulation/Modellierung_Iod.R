'IOD 131 MODELLIERUNG'
'0-Vorbereitung der R-Umgebung'

#Modellierungsordner festlegen
setwd("D:/Studium landau/BA Arbeit/Umsetzung in R") '!!!!!!!!!!!!!!!!!!!!!!!!!! hier muss Verzeichnis anggepasst werden, sodass Ordner "Meteorologie" in gleichem Ordner liegt!!!'
...
# 1.WINDEINFLÜSSE
  ...
  nc_fname1 <- "Meteorologie/Winddaten.nc"
...
# 2. REGENEINFLÜSSE
  ...
  #Laden der NCDF4 Datei 
  nc_fname2 <- "Meteorologie/Regendaten.nc"
...
#Laden der NCDF4 Datei 
nc_fname3 <- "Meteorologie/vertikale Windkomponente.nc"
...
TESTABLAGERUNG_bel<-c()#testzeug %&%&2
  ...
  #Belastung
  Gesamtbelastung<-5.48665E16 #Iod-131 Freisetzung in den ersten 7 Tagen (Terada 2012)
  ...
  Zeitverteilung_Freisetzung<-c(
                                9.25E13, 5.1E13, 1.6765E15,
                                2.52E+14, 2.52E+14, 2.52E+14,
                                2.52E+14, 2.52E+14, 2.52E+14,
                                2.52E+14, 2.52E+14, 2.52E+14,
                                2.04E14, 1.08E14, 1.08E14,
                                1.08E+14, 1.58E+15, 6.90E13,
                                6.90E+13, 6.90E+13, 3.26E+15,
                                1.05E15, 1.05E15, 6.35E+15,
                                3.16E15, 8.08E+15, 8.21E+14,
                                6.30E+14, 6.30E+14, 6.30E+14,
                                6.30E+14, 6.30E+14, 6.30E+14,
                                6.30E+14, 6.30E+14, 6.30E+14,
                                6.30E+14, 6.30E+14, 6.30E+14,
                                1.23E15, 1.23E15, 1.23E15,
                                1.23E15, 1.23E15, 1.23E15,
                                1.23E15, 1.23E15, 1.23E15,
                                1.23E15, 1.23E15, 1.23E15,
                                1.23E15, 1.23E15, 1.23E15,
                                1.23E15, 1.23E15)   #Werte aus Terada 2012 in 3h-Abschnitt (beachte UTC Zeit!) 
  Verteilung<-c(0.8,0.2) #gas, partikel (Anteil)
  v_trocken<- c(5E-3,1E-3) #gas, partikel (m/s)
  nass_a<-c(4E-5,7E-5)#gas, partikel
  nass_b<-c(0.6,0.69) #gas, partikel 
...
for (Freisetzungszeitpunkt in Beginn_Betrachtung_Index:(Ende_Betrachtung_Index)){ #läuft 55 Zeiteinheiten durch in meinem Ursprungsbeispiel, direkt auf Eintrag in NCDF4 Datei zugreifbar
  ...
  for (Art in 1: 2){ #Partikel oder gasförmig
    
'4-Initilaisierung Anfangsparameter für EINE ZEITEINHEIT IN EINER BESTIMMTEN HÖHE'

    #Menge an Radioaktivität
    Belastung<- Zeitverteilung_Freisetzung[Freisetzungszeitpunkt-Beginn_Betrachtung_Index+1]*Verteilung[Art] #ANGEPASST IOD !
...
'9-Trockene Deposition (und Neudefinition des Ortes nach Absinken)'
      v_TD<-v_trocken[Art]*3*3600 #ANGEPASST IOD !
      Trajektorie1[3]<-Trajektorie1[3]-v_TD
      Trajektorie1_Grad[3]<-Trajektorie1_Grad[3]-v_TD

'10-Nasse Deposition'
      Ablagerungsanteil<-nass_a[Art]*3*3600*(var3[1+2*(Koordinaten[2]-Koordinate_W),1+2*(Koordinate_N-Koordinaten[1]),Entstehungszeitpunkt+Dauer-Zeitanpassung_Regen]*3*1000)^nass_b[Art]  #pro 3h  #ANGEPASST IOD !
